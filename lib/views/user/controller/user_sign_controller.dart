...
class UserSignController extends BaseRequestController {
  ///
  ///
  ...
  void exchangeCoupon() {
    ...
    UserInfoRepository.couponExchange().then((value) {
      ...
      BalanceInstance().incrCoupon = value.coupon;
      EasyLoading.show(
        dismissOnTap: true,
        indicator: Column(
          children: [
  ...
  void signAction({required Function success}) {
    ...
    signing = true;
    update();
    UserInfoRepository.userSign().then((value) {
      ///签到信息更新
      ...
      ///截取前10条日志
      logs = logs.sublist(0, min(limit, logs.length));
  ...
  Future<void> request() async {
    ...
    Future<void> signAsync = UserInfoRepository.querySignInfo().then((value) {
      signInfo = value;
    });
    Future<void> logsAsync = UserInfoRepository.querySignLogs({
      'page': page,
      'limit': limit,
    }).then((value) {
      logs
